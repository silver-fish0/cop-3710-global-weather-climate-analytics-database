# COP-3710-Global-Weather-Climate-Analytics-Database

Global Weather & Climate Database

Application Domain:

Database for storing weather and climate measurement data (temperature, wind speed, precipitation). Users can access the database to view, filter, and compare measurements across different locations and time periods.

High-level goals:

- Store location data such as city, state, elevation.
- Store weather and climate measurement readings with timestamps and units.
- Use SQL to generate summary results.
  
Data sources:

Global Historical Climatology Network daily - https://www.ncei.noaa.gov/products/land-based-station/global-historical-climatology-network-daily
Global Data Lab - https://globaldatalab.org/climate/

User:

- Admin
- Students
- Researchers
- Viewers
